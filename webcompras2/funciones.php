...
function mysqlcdc($nif,$feci,$fecf ) {											
	...
	try {
		...
		$stmt = $conn->prepare("SELECT nombre, precio, unidades, fecha_compra, hora FROM compra, producto
		...
		if(empty($res)){
			...
			foreach($res as $dat){
				...
				echo "Ha comprado ".$dat["unidades"]." unidades de ".$dat["nombre"]." con un precio cada uno de  ".$dat["precio"]. "€ y un total de ".$total."€. El día: ".$fc." a las ".$hora."<br>";
			}
		}
...
function mysqlcpro($nif,$prod,$cant,$numalm) {
	...
	try {
		...
		// prepare sql and bind parameters
		$stmt = $conn->prepare("INSERT INTO compra (NIF,id_producto,fecha_compra,unidades,hora) VALUES (:NIF,:id_producto,:fecha_compra,:unidades,:hora)");
		$stmt->bindParam(':NIF', $dni);
		$stmt->bindParam(':id_producto', $id);
		$stmt->bindParam(':fecha_compra', $fecc);
		$stmt->bindParam(':unidades', $ud);
		$stmt->bindParam(':hora', $hor);
  
		//Para saber la fecha de la compra
		$fec = date('y/m/d') ;
		$h = new DateTime("now", new DateTimeZone('Europe/Madrid')); 
		$h2 = get_object_vars($h);
		$h3 = $h2['date'];
		$hora =  substr($h3, 11, 8);
...
function errorrdc($usu,$clv){
	...
	if ($usu == "") {
		echo "Tienes que añadir tu nombre como usuario"."<br>";
		$error = true;
	}else{
		if ($clv == "") {
			echo "Tienes que añadir tu clave (su apellido al revés)."."<br>";
			$error = true;
		}else{
			for ($i=0; $i <= count($usurdc)-1; $i++){
			if ($usu == $usurdc[$i]){
				$usuv = true;
			}
		}
	...
	if ($clvv == false){
		echo "Clave no válida (la clave es su apellido al revés)."."<br>";
	}
...
function clvrdc(){
	...
	try {
		...
		$stmt = $conn->prepare("SELECT apellido FROM cliente");
		$stmt->execute();
		...
		$cont = 0;
		foreach($stmt->fetchAll() as $row) {
			$clvrdc[$cont] = strrev($row["apellido"]);
			$cont++;
		}
		return $clvrdc;
	}
...
function sesionl($usu){
	...
	if(isset($_COOKIE[$cookie_name])) {
		...
		echo "El usuario no se ha registrado"."<br>";
		echo "<a href='comregcli.php'>Registrarse</a>";
	}

}

function refrescar(){
	header( "refresh:2; url=comlogincli.php" );
}

//comprocli

function añadircesta($prod,$cant,$usu,$nprod){

	$cookie_name = "cesta_".$usu;
	//unset($_SESSION[$cookie_name]);
	if(!isset($_SESSION[$cookie_name])) {
	
		//$cookie_value[0][0] = $prod."|".$cant;
		$cookie_value=array(array($prod,$cant,$nprod));

		$_SESSION[$cookie_name] = $cookie_value;
		//$_COOKIE[$cookie_name] = $cookie_value;
		//setcookie($_COOKIE[$cookie_name]);
		//setcookie($cookie_name, serialize($cookie_value), time() + (86400 * 3), "/");
		//setcookie($cookie_name, $cookie_value, time() + (86400 * 3), "/"); // 86400 segundos = 1 día
		echo "Cesta creada."."<br>";
		
		var_dump($_SESSION[$cookie_name]);
	} else {
		
		$cookie_value=array($prod,$cant,$nprod);
		
		array_push($_SESSION[$cookie_name], $cookie_value);
		array_multisort($_SESSION[$cookie_name]);
		//$cookie_value[1][1] = $prod."|".$cant;
		//$_COOKIE[$cookie_name],serialize($cookie_value);
		//setcookie($cookie_name, serialize($cookie_value), time() + (86400 * 3), "/"); // 86400 segundos = 1 día
		echo "Ya existe una cesta."."<br>";
		var_dump($_SESSION[$cookie_name]);
		/*$arrses = $_SESSION[$cookie_name];
		
		foreach ($arrses as $valor => $var2) {
...
function mostrarcesta($usu) {
	...
	for($i=0; $i<=count($cesta)-1; $i++)
	  ...
	  echo "<input type='checkbox' name='".$cesta[$i][0]."' value='".$cesta[$i][0]."'>";
	  echo " Producto: ".$cesta[$i][2];
	  echo " -> Cantidad: ".$cesta[$i][1]."<br><br>";
      }
}

function eliminarprod($nprod,$cant,$numalm){
	...
	try {
		...
		$stmt = $conn->prepare("UPDATE almacena SET cantidad = cantidad-'$cant' WHERE
			id_producto = '$nprod' AND num_almacen = '$numalm' ");
										
		$stmt->bindParam('$cant', $cantidad);
		$stmt->bindParam('$nprod', $id_producto);
		$stmt->bindParam('$numalm', $num_almacen);
		$stmt->execute();
...
function agregarcompra($nif,$prod,$cant) {
...
function darnif($usu){
	...
	try {
		...
		$stmt = $conn->prepare("SELECT nif FROM cliente WHERE nombre ='$usu' ");
		$stmt->execute();
